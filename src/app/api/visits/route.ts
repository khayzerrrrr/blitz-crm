...
export async function GET(req: NextRequest) {
  ...
  const [data, total] = await Promise.all([
    prisma.visit.findMany({
      where, include: { school: { select: { id: true, name: true, city: true, latitude: true, longitude: true } }, user: { select: { id: true, name: true } } },
      skip: (page - 1) * perPage, take: perPage, orderBy: { createdAt: 'desc' },
    }),
...
export async function POST(req: NextRequest) {
  ...
  const visit = await prisma.visit.create({ data: { ...body, userId: session.user.id } });
